...
`minimaxPreset()` ships provider `minimax`, exact model `MiniMax-M3`, and
`MINIMAX_API_KEY` at the Anthropic-compatible regional endpoint
`https://api.minimaxi.com/anthropic` (CN default) or
`https://api.minimax.io/anthropic` (INTL). Its manifest explicitly wires
`vision`. MiniMax-M3 natively accepts image/video content blocks on that same
